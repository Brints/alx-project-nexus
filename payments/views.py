...
class PaymentViewSet(viewsets.GenericViewSet):
    ...
    def initialize(self, request):
        ...
        try:
            ...
            logger.info(f"Payment initialized for {user.email} - Ref: {data['reference']}")
    ...
    def _process_payment_verification(self, tx_ref, user, from_webhook=False):
        ...
        if txn.status == 'SUCCESS':
            ...
            logger.info(f"{'Webhook' if from_webhook else 'Verify'}: {message} for {tx_ref}")
            if from_webhook:
                return Response(status=status.HTTP_200_OK)
            return Response(
                {
        ...
        if verification_data.get('status') == 'success':
            ...
            logger.warning(f"Payment verification failed for {tx_ref}: {verification_data}")
